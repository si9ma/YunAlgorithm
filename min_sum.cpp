...
namespace si9ma{
    ...
    template <int len>
    int MinSum::min_sum_bad(int (&arr)[len]) {
        int min_sum = 0;
    ...
    template <int len>
    int MinSum::min_sum_good(int (&arr)[len]) {
    ...
    // 时间复杂度 --> T(N)
    template <int len>
    int MinSum::process(int (&arr)[len], int L, int R) {
        if (L == R)
            return 0;
    ...
    template <int len>
    int MinSum::merge(int (&arr)[len], int L, int M, int R) {
